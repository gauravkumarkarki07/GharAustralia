...
export default function PersonalDetails() {
  ...
  const SaveChanges=async(e)=>{
    e.preventDefault();
    const firebaseResponse=await uploadImageToFireBase(image,'tenant','profilePicture',session._id);
    const updatedData = {...data, profilePicture: firebaseResponse};
    const response=await PutRequest('/ghar/tenant/updateProfileDetails',updatedData);
    if(response){
      return dispatch(loginTenant({sessionDetails:response.tenantDetails}));
    }
  }
